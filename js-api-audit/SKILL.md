...
- 对所有 `POST` 且 `Content-Type: application/json` 或 body 形态未知的接口，除基于代码恢复出的参数请求外，至少额外执行一次空 JSON `{}`
  请求，用于识别“空体也会返回业务数据/业务状态”的接口，不能因为未还原参数而跳过这一步。
...
4. 对 `POST` JSON 接口，若参数无法稳定还原、或希望先判断是否存在默认列表/默认响应，至少补发一次空 JSON `{}`；该请求可与 `NoAuth`、`LowPriv` 等模式组合记录。
5. 写操作/高副作用接口使用 canary 和低副作用参数：`id=-1/0/999999999`、`name=audit_probe_{timestamp}`、`remark=security_test_noop`、`dryRun=true`、`validateOnly=true`、`preview=true`、`force=false`。
6. 文件上传、导入、执行任务类接口优先发送缺文件、空文件名、非法 ID 或 `dryRun/preview` 请求，观察是否先鉴权、是否返回业务成功、是否暴露路径/任务 ID；不要上传真实敏感文件。
...
| PostEmptyJsonProbe | 对 `POST` JSON 接口额外发送空 JSON `{}`，确认空体时的鉴权、默认业务响应和错误语义 |
...
| PostEmptyJsonProbe | 对 `POST` JSON 接口发送 `{}`，验证空体时是否直接返回业务数据、默认分页结果、业务错误细节或成功状态 |
...
- [ ] 对所有 `POST` JSON 或 body 形态未知接口，已额外尝试空 JSON `{}` 探测，或明确说明为何不能发送该请求。
